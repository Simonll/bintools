...
class posterior_GTR(posterior):
    ...
    @classmethod
    def parse_mcmc(cls, mcmc_path: Path, burnin: int = 0) -> Optional["posterior_GTR"]:
        """
        parses mcmc
        """
        list_of_sampleID: List[int] = []
    ...
    @classmethod
    def read(cls, input_file: Path) -> Optional["posterior_GTR"]:
        list_of_sampleID: List[int] = []
        list_of_trees: List[str] = []
        list_of_phi: List[List[float]] = []
        list_of_rho: List[List[float]] = []
